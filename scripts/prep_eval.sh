...
if [ -z "$input_file" ]; then
  echo "Usage: bash prep.sh --xqy foo.xqy [--database database_name]"
  exit 1
fi
...
# Create the output XQuery file with contents and database embedded in the template
cat << EOF > "$output_file"
...
echo "Created $output_file with embedded contents from $input_file and database set to $database"
